...
fn debug_determine_bag() {
    let mut structure = vec![];
    // let commands = vec![[1, 1], [1, 2], [1, 3], [2, 1], [2, 2], [2, 3]]; // queue
    // let commands = vec![[1, 2], [1, 1], [2, 1], [2, 2]]; // stack
    // let commands = vec![[1, 2], [1, 5], [1, 1], [1, 3], [2, 5], [1, 4], [2, 4]]; // priority queue
    // let commands = vec![[1, 1], [1, 2], [1, 3], [2, 3], [2, 2], [2, 1]]; // not sure
    let commands = vec![[1, 1], [2, 2]]; // impossible
...
fn main() {
    ...
    for _line in input.lock().lines().map(|_line| _line.unwrap()) {
        ...
        if command.len() == 1 {
            ...
            if !structure.contains(&command[1]) {
                is_stack = false;
                is_queue = false;
                is_priority = false;
            } else {
